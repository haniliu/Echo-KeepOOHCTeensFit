...
if ( ! class_exists( 'uncover_Info' ) ) {
	...
	class uncover_Info {
		 ...
		 * Single instance.
		 *
		 * @var uncover_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof uncover_Info ) ) {
				self::$instance = new uncover_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'uncover' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'uncover' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-info';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'uncover' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'uncover' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'uncover_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'uncover' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_uncover_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_uncover_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_tabs() {
			...
			echo wp_kses('<h2 class="nav-tab-wrapper wp-clearfix">', $allowed_html);
			...
			echo wp_kses('<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '&tab=' . esc_attr( $key ) . '" class="nav-tab' . esc_attr( $current_class ) . '">' . esc_html( $tab ) . '</a>', $allowed_html);				
			}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'uncover' ), esc_html($method) );
			}
		}
		...
		public function useful_plugins() {
			...
			<div class="wt-section wt-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'uncover' ); ?></a>
				<?php endif; ?>
		...
		public function assets( $hook ) {
			...
			wp_enqueue_style( 'uncover-info', get_template_directory_uri() . '/admin/about/css/info' . $min . '.css', array(), '1.0.0' );
			wp_enqueue_script( 'uncover-info', get_template_directory_uri() . '/admin/about/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'uncover_dismiss_status', true );
			?>
		...
		public function render_quick_links() {
			...
			$allowed_html = uncover_allowed_html();
			echo wp_kses('<p class="quick-links">', $allowed_html);				
			...
			echo wp_kses('<a href="' . esc_url( $link['url'] ) . '" class="button ' . esc_attr( $button_type ) . '" target="_blank">' . esc_html( $link['text'] ) . '</a>', $allowed_html);				
				}
			echo wp_kses('</p>', $allowed_html);								
			}
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'uncover_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'uncover_dismiss_status', 1 );
